...
| P0-6 | コンテンツデータの空ファイル雛形を作成 | `src/data/` 配下7ファイルが型付きで存在する | 完了 |
| P0-7 | デザイントークン（色・余白・フォント）を `globals.css` に定義 | 限定色数のパレットが変数化されている | 完了 |
| P0-8 | `GameShell` / `PixelWindow` / `PixelButton` を実装 | ダミーページでRPG風の枠と押せるボタンが出る | 完了 |
| P0-9 | `basePath` を考慮した画像パスヘルパーを実装 | ローカルと本番の双方で画像が表示される | 完了 |

### P0-4 が進行中である理由

配信ワークフローは `.github/workflows/deploy.yml` にあるが、リポジトリの Pages が有効化されていない。
このリポジトリは非公開であり、非公開リポジトリの Pages は有料プランでのみ使える。

そのため、次のどちらかを選ばないと P0-4 は完了にできない。

- リポジトリを公開に切り替える
- Pages を使える有料プランで有効化する

どちらも実装では解消できず、リポジトリまたはアカウントの設定変更を伴う。
有効化できたら、`deploy.yml` の `on:` に `push: branches: [main]` を戻す。

P0-4 が通るまで、Phase 0 と Phase 1 の完了ゲートのうち本番URLを要する項目は保留となる。
...
- 決定: [ADR-0003](decisions/0003-master-data-types.md)（マスターデータの型を4件追加）
