...
当初はリポジトリが非公開で Pages を有効化できず、`deploy.yml` を手動実行のみに絞っていた。
その後リポジトリが公開に切り替わり、Pages がソース `GitHub Actions`（`build_type: workflow`）で有効化された。

これを受けて `deploy.yml` の `on:` に `push: branches: [main]` を戻した。
公開URLは https://tabio.github.io/sumo-quest/ 。

状態は、この変更が main にマージされて初回の自動配信が成功した時点で `完了` とする。
完了ゲートおよび他フェーズで保留していた本番URLでの確認も、そのあとで実施できる。
